#Call with python3 ./src/histogram_maker.py [data style T/H/L] [filename w/ extension]  [(optional) name of plot to show]
import matplotlib.pyplot as plt
import numpy as np
import csv
import sys
# S-stack distance H-frequency histogram,entropy&frequence L-stride run frequency T-stack distance&frequency 
#L-streak size&frequence
dataStyle = sys.argv[1]
with open(sys.argv[2], "r", encoding="utf-8") as source:
    reader = csv.reader(source,delimiter=',')
    for i, row in enumerate(reader): #specify which row to plot (row-1)
        if i == 13:
            #print("This is the line.")
            #print(row)
            break

    if dataStyle == 'T': #stack distance vs. frequency
        if len(sys.argv) > 3: # has plot style and name of the plot arguments 
            row = next(reader)
        ...
        if len(sys.argv) > 4:
            ...
            for row in reader:
                ...
                for entry in row[2:]:
                    item_list.append(entry[:entry.find(':')])
                    freq_list.append(int(float(entry[entry.find(':')+1:]))) #add float conversion
                old_order = list(zip(item_list,freq_list)) 
                #print(int(old_order[0][0]))
                old_order.sort(key=lambda y: y[0])
                new_order = [list(t) for t in zip(*old_order)]
                item_list = new_order[0]
                freq_list = new_order[1]
                if len(sys.argv) > 3 and sys.argv[3] == "L":
        ...
        if len(sys.argv) > 4:
            ...
            for row in reader:
                ...
                for i in range(2,len(row)):
                    item_list.append(str(i-1))
                    freq_list.append(int(float(row[i])))
                if len(sys.argv) > 3 and sys.argv[3] == "P":
                    item_list = [int(t) for t in item_list]
                    for i in range(1,len(freq_list), -1):
                        freq_list[i] /= freq_list[i-1]

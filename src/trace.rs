...
impl<I: Item> From<Vec<I>> for Trace<I> {
    fn from(trace: Vec<I>) -> Self {
        ...
        Self { inner: trace , strides} // inner holds the trace 
    }
}
...
impl<I: Item> Trace<I> {
    ...
    /// Calculate the stride histogram of the strides based on a given condition.
    ///
    /// ```
    /// # use std::collections::HashMap;
    /// # use cache_sim::{Trace, NoCondition};
    ...
    #[must_use]//?????? implement right?? 
    pub fn stack_distances(&self, paging_model: bool) -> StackDistance {
        let mut distances = vec![Some(0); self.len()]; //create a vec with size self.len() for values Some(0)
    ...
    pub fn average_entropy(&self, prefix: usize) -> f64{
        ...
        for i in prefix..self.inner.len() {
        	*freqs.entry(&self.inner[(i-prefix)..i]).or_insert(0) += 1; //increment freq every time it sees the same slice 
			*distributions.entry(&self.inner[(i-prefix)..i]).or_insert(HashMap::default()).entry(self.inner[i]).or_insert(0) += 1; //only insert self.inner[i]?? 
        }
        dbg!("item freqs done");
		let mut sum: f64 = 0.0;
		...
		for (seq,count) in freqs{
			if let Some(hist) = distributions.get(&seq){
				sum += ((count as f64)/((self.len()-prefix) as f64))*entropy(hist); //histogram: only [i:count]?? one value 
			} //why -prefix?
		}
    ...
    /// Get the number of unique sequences of items in the trace that are exactly length long -> number of unique items
...
#[must_use]
pub fn entropy<I: Item, H: std::hash::BuildHasher>(histogram: &HashMap<I, u32, H>) -> f64 { //wrong calculation???
    let total = f64::from(histogram.values().sum::<u32>()); //let total: f64 = histogram.keys().len()
    -histogram
        .values()
...
pub fn trace_strides<I: Item>(trace: &Vec<I>) -> Vec<i64>{
	...
	for i in 1..trace.len(){
		strides.push(trace[i].id() as i64 - trace[i-1].id() as i64); //difference of address 
	}
	strides
}
